...
from model_skeleton import ParserModel, TwoHiddenParserModel, ReluParserModel
...
def train(save_dir='new_saved_weights',
          parser_name='parser',
          num_epochs=5,
          max_iters=-1,
    ...
    optimizer = torch.optim.AdamW(parser.parameters(), lr = config.lr)

    print("Enter Main Training Loop!")

    total_training_loss_history = []
    total_training_acc_history = []

    for epoch in range(1, num_epochs + 1):

        training_iter_history = []
        ...
        for i, (train_x, train_y) in enumerate(get_minibatches([dataset.train_inputs,
            ...
            word_inputs_batch = torch.Tensor(np.array(word_inputs_batch))
            pos_inputs_batch = torch.Tensor(np.array(pos_inputs_batch))  # TODO
            dep_inputs_batch = torch.Tensor(np.array(dep_inputs_batch))
            ...
            labels = np.argmax(np.array(train_y), axis=1)  # TODO

            # Convert the label to pytorch's tensor
            labels = torch.LongTensor(labels)  # TODO
            ...
            outputs = parser.forward(word_inputs_batch, pos_inputs_batch, dep_inputs_batch)  # TODO
            ...
            if i % print_every_iters == 0:
                ...
                training_iter_history.append(i)
                training_loss_history.append(loss.item())
                training_acc_history.append(int((outputs.argmax(1)==labels).sum())/len(labels))
                total_training_loss_history.append(loss.item())
                total_training_acc_history.append(int((outputs.argmax(1)==labels).sum())/len(labels))
        print("End of epoch")

        # plot loss and accuracy
        plt.figure()
        sns.lineplot(x=training_iter_history, y=training_loss_history)
        plt.title("Loss vs. Iteration")
        plt.xlabel("Iteration")
        plt.ylabel("Loss")
        loss_file_name = "epoch" + str(epoch) + "_loss.jpg"
        plt.savefig(loss_file_name)

        plt.figure()
        sns.lineplot(x=training_iter_history, y=training_acc_history)
        plt.title("Accuracy vs. Iteration")
        plt.xlabel("Iteration")
        plt.ylabel("Accuracy")
        acc_file_name = "epoch" + str(epoch) + "_acc.jpg"
    ...
    # plot loss and accuracy
    total_training_epoch_history = [50*k for k in range(len(total_training_acc_history))]
    plt.figure()
    sns.lineplot(x=total_training_epoch_history, y=total_training_loss_history)
    plt.title("Loss vs. Iteration")
    plt.xlabel("Iteration")
    plt.ylabel("Loss")
    ...
    plt.figure()
    sns.lineplot(x=total_training_epoch_history, y=total_training_acc_history)
    plt.title("Accuracy vs. Iteration")
    plt.xlabel("Iteration")
    plt.ylabel("Accuracy")
    plt.savefig("acc_vs_epoch.jpg")
